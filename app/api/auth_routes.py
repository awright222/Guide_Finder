...
#         if user and user.check_password(password):
#             login_user(user)
#             return jsonify({'user': user.to_dict(), 'role': 'user'})
        
#         # Check if the email belongs to a guide
#         guide = Guide.query.filter_by(email=email).first()
#         if guide and guide.check_password(password):
#             login_user(guide)
#             return jsonify({'user': guide.to_dict(), 'role': 'guide'})
